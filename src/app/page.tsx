    ...
    <div className="min-h-screen space-y-32 md:space-y-40 pb-40 bg-neutral-soft">
      ...
      <section className="relative overflow-hidden min-h-[50vh] flex items-center">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-8 py-24 text-center">
          <p className="inline-block text-xs uppercase tracking-wider text-metal-gold-light mb-6">
            Bespoke Jewellery Atelier
          </p>
          <h1 className="font-serif font-normal text-5xl md:text-6xl lg:text-7xl leading-tight text-white max-w-4xl mx-auto tracking-tight mb-8">
      ...
      <section className="mx-auto max-w-7xl px-8">
        <div className="grid gap-8 md:grid-cols-3">
          <div className="text-center space-y-3 p-8">
            <h3 className="font-serif text-2xl text-deep-black">Handcrafted</h3>
            <p className="text-sm text-deep-black/60 leading-relaxed">
              Every piece shaped by hand in Kandy, Sri Lanka
            </p>
          </div>
          <div className="text-center space-y-3 p-8 border-x border-metal-gold/20">
            <h3 className="font-serif text-2xl text-deep-black">Sri Lankan Gems</h3>
            <p className="text-sm text-deep-black/60 leading-relaxed">
              Ethically sourced coloured gemstones from local suppliers
            </p>
          </div>
          <div className="text-center space-y-3 p-8">
            <h3 className="font-serif text-2xl text-deep-black">Affordable Custom</h3>
            <p className="text-sm text-deep-black/60 leading-relaxed">
      ...
      <section id="process" className="mx-auto max-w-7xl px-8 scroll-mt-24">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          {processSteps.map((step, index) => (
            <div key={step.title} className="space-y-4 p-6">
              <span className="text-5xl font-serif text-metal-gold/40">{String(index + 1).padStart(2, '0')}</span>
              <h3 className="font-serif text-xl text-deep-black">{step.title}</h3>
              <p className="text-sm leading-relaxed text-deep-black/60">{step.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 max-w-5xl mx-auto">
          ...
          <div className="space-y-4">
            {faqs.slice(0, 4).map((faq) => (
              <div key={faq.question} className="p-8 space-y-4 border border-metal-gold/10 rounded-2xl hover:border-metal-gold/30 transition-colors">
                <h3 className="text-base font-semibold text-deep-black">{faq.question}</h3>
                <p className="text-sm leading-relaxed text-deep-black/60">{faq.answer}</p>
              </div>
            ))}
          </div>
        </div>
