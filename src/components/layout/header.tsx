...
const CurrencySelector = dynamic(
  () => import("@/components/ui/CurrencySelector"),
  { ssr: false },
);
...
export default function Header() {
  ...
  const [suggestions, setSuggestions] = useState<
    Array<{ name: string; slug: string; price: number }>
  >([]);
  ...
  const fetchSuggestions = useCallback(async (q: string) => {
    ...
    try {
      ...
      setSuggestions(
        prods.map((p: any) => ({ name: p.name, slug: p.slug, price: p.price })),
      );
      setHighlighted(-1);
    } catch {
      setSuggestions([]);
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-3 sm:px-6 py-2 sm:py-3 gap-2 sm:gap-4 md:gap-6">
        ...
        <div className="flex items-center justify-end gap-2 sm:gap-3 md:gap-5 flex-1">
          ...
          >
            ...
            {cartCount > 0 && (
              <span
                className="absolute -top-1 -right-1 sm:top-0 sm:right-0 flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-foreground text-[10px] sm:text-xs font-medium text-white"
                aria-hidden="true"
              >
                {cartCount}
              </span>
          ...
          >
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth={1.5}
                d="M3.75 6.75h16M4 12h16M4 18h16"
              />
            </svg>
          </button>
        </div>
